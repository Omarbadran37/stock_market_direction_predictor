...
sp_500_1 <- sp_500 %>%
  filter(!symbol %in% c("CEG","OGN","AMCR","TTWO","CTVA","DOW","UA"))

start_date <- Sys.Date()-366
...
nas <- nas %>%
  filter(market.cap >= 1000000000 & ipo.year <2021)
...
#if looking at sp500 stocks use below filter

sp_500_1 <- sp_500_1 %>%
  filter(!symbol %in% c("CEG","OGN","AMCR","TTWO"))
...
# 
 test <- sp_500_1 %>%
   ...
   na.omit()
# 
# test <- inner_join(x=test,y=ten_bil_companies %>%
#                      select(symbol),
#                    by="symbol")
...
saveRDS(rf_mod_15,"SP500_rfml_lag4.rds")
...
stock_buy_15 <-stock_pred_15_rf_symbol %>%
  group_by(symbol)%>%
  filter(.pred_buy >=.60)%>%
  arrange(date,desc(.pred_buy))

stock_buy_15$diff_lag <- stock_buy_15$date - lag(stock_buy_15$date)

stock_buy_15 <-stock_pred_15_rf_symbol %>%
  group_by(date)%>%
  filter(date >= "2021-01-01" )%>%
  slice_max(.pred_buy, n=40)
...
stock_buy_15 <- left_join(x=stock_buy_15, y= full_index%>%
                            select(symbol,company,market.cap),
                          by = "symbol")

ten_bil_companies <- full_index %>% filter(market.cap >= 10000000000)


rf_mod_5 <- readRDS("stock_pick_model_5day_rf_new.rds")

stock_ml <- date_filter%>%
  ungroup(symbol)%>%
...
stock_pred_5_rf_symbol  <- bind_cols(stock_pred_5_rf, select(date_filter, symbol, close,target_5,target_direction_5))

stock_pred_5_rf_symbol$action <- if_else(stock_pred_5_rf_symbol$.pred_buy >=.50,"buy","sell")

stock_buy <-stock_pred_5_rf_symbol %>%
  filter(action =="buy")%>%
  select(symbol)


stock_sell <-stock_pred_5_rf_symbol %>%
  filter(action =="sell")

# stock_buy<-date_filter %>%
#   filter(rsi_14 <= 50 | rsi_14_lag1 <= 50 | rsi_14_lag2 <= 50,
#          hma_9_lag_1>=1, hma_9_lag_3>=1, 
#          sar_ratio >=1, diff_short>=0)%>%
#   select(symbol)
...
stock_call_final <- stock_pred_5_rf_symbol %>%
  ...
  slice_max(.pred_buy, n=100)

stock_call_final <- inner_join(x=stock_call_final, y=full_index %>%
                                 select(symbol, company, industry), 
                               by= "symbol")

stock_call_final$five_percent_increase <- stock_call_final$close + (stock_call_final$close*.05)
stock_call_final$ten_percent_increase <- stock_call_final$close + (stock_call_final$close*.10)
 
stock_call_final <-stock_call_final %>%
select(.pred_buy, symbol,close,five_percent_increase,ten_percent_increase,company,industry)


last_week <- sp_500_1 %>%
  filter(date=="2022-02-11")

last_week_stock_ml <- last_week%>%
  ungroup(symbol)%>%
  select(-c(symbol,date, open, high, low, close, volume, adjusted, return_lag1,
            return_daily, close_shift_5, target_5, close_shift_10, target_10,
            target_direction_10, close_shift_15,target_15, target_direction_15,close_shift_20,
...
last_week_stock_pred_5_rf <-  predict(rf_mod_5, last_week_stock_ml, type = "prob")

last_week_stock_pred_5_rf_symbol  <- bind_cols(last_week_stock_pred_5_rf, select(last_week,date, symbol, close,target_5,target_direction_5))

last_week_stock_pred_5_rf_symbol$action <- if_else(last_week_stock_pred_5_rf_symbol$.pred_buy >=.50,"buy","sell")

last_week_stock_buy <-last_week_stock_pred_5_rf_symbol %>%
  filter(action =="buy")
