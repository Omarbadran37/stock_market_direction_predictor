...
sp_500_1 <- sp_500 %>%
  filter(!symbol %in% c("CEG","OGN","AMCR","TTWO","CTVA","DOW"))
...
sp_500_1$sar_ratio_lag20 <- lag(sp_500_1$sar_ratio, n=20)
 ...
 sp_500_1 <- sp_500_1 %>%
   filter(!symbol %in% c("AEVA","AMCR","BTRS","DKNG","LAZR","LPRO","MAXR","ARQQ"))
...
stock_pred_5_rf <-  predict(rf_mod_5, stock_ml_5, type = "prob")
...
rf_mod_10 <- readRDS("sp_500_rfml_10day.rds")

stock_ml_10 <- date_filter%>%
  ungroup(symbol)%>%
  select(-c(symbol,date, open, high, low, close, volume,  return_lag1,
...
stock_pred_10_rf <-  predict(rf_mod_10, stock_ml_10, type = "prob")

stock_pred_10_rf_symbol  <- bind_cols(stock_pred_10_rf, 
...
stock_pred_10_rf_symbol$five_percent_increase <- stock_pred_10_rf_symbol$close + (stock_pred_10_rf_symbol$close*.05)
stock_pred_10_rf_symbol$ten_percent_increase <- stock_pred_10_rf_symbol$close + (stock_pred_10_rf_symbol$close*.10)
...
sum(stock_buy$target_5)
sum(stock_buy$target_10)
sum(stock_buy$target_15)
...
stock_call$this_week_option <- if_else((stock_call$start_date1-as.Date(friday))<=14,1,0)

stock_call <- stock_call %>%
  filter(this_week_option ==1)

stock_call_final <- stock_pred_10_rf_symbol %>%
  filter(symbol %in% stock_call$column_label)%>%
  slice_max(.pred_buy, n=30)


stock_train_15 <- date_filter%>%
  select(-c(symbol,date, open, high, low, close, volume, return_lag1,
            return_daily, close_shift_5, target_5,target_direction_5, close_shift_10, target_10,
...
rf_mod_15 <- readRDS("sp_500_rfml_15day.rds")

stock_pred_15_rf <-  predict(rf_mod_15, stock_train_15, type = "prob")

stock_pred_15_rf_symbol  <- bind_cols(stock_pred_15_rf, 
                                      select(date_filter, date,
                                             symbol, close,target_10,target_direction_10,
                                             target_5,target_direction_5, target_15,target_direction_15, 
...
stock_buy_15_yesterday <- stock_pred_15_rf_symbol%>%
  group_by(date)%>%
  filter(date >= "2021-01-01" )%>%
  slice_max(.pred_buy, n=20)

stock_buy_today <- anti_join(x=stock_buy_15, y= stock_buy_15_yesterday, by="symbol")
