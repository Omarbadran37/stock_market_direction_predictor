...
sp_500_1 <- sp_500_1 %>%
  filter(!symbol %in% c("TCN","ACT","CRDO","LBTYB","AMLX"))
...
stock_pred_rf_symbol<- stock_pred_rf_symbol %>%
  select(date,symbol,open, close,pred_buy_3,pred_buy_5,pred_buy_10,pred_buy_15,pred_buy_20,
         target_direction_3,target_direction_5,target_direction_10,
         target_direction_15,target_direction_20,avg ,target_3, target_5, target_10, target_15,target_20)


ggplot(stock_pred_rf_symbol, aes(x=as.factor(target_direction_3), y=pred_buy_3, fill = target_direction_3)) + 
  geom_violin()+ geom_jitter(shape=16, position=position_jitter(0.2))

write.csv(stock_pred_rf_symbol, "watchlist.csv")


test <- anti_join(x=full_index %>%
                    select(symbol), y = sp_500%>%
                    select(symbol), by="symbol")
