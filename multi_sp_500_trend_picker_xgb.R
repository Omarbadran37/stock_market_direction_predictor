...
l.out <- yf_get(tickers = full_index$symbol,
...
gb_mod_3 <- boost_tree(
  ...
  min_n = 15,
  tree_depth = 15,
  learn_rate = 0.0265593712625742,
  loss_reduction = 0.315988707733803,
  sample_size = 0.795753022423014                        ## step size
) %>% 
...
saveRDS(gb_mod_3,"SP500_gbml_trendpick_3_aapl.rds")

library(vip)
gb_mod_3 %>%
  vip(geom = "point",num_features = 50)

test_pred_3_rf <- predict(gb_mod_3, stock_test_3, type = "prob") %>%
  bind_cols(predict(gb_mod_3, stock_test_3)) %>%
...
saveRDS(gb_mod_5,"SP500_gbml_trendpick_5_aapl.rds")

library(vip)
gb_mod_5 %>%
  vip(geom = "point",num_features = 50)

test_pred_5_rf <- predict(gb_mod_5, stock_test_5, type = "prob") %>%
...
stock_test_10 <- test%>%
  ungroup(symbol)%>%
  select(-c(symbol,date, open, high, low, close, volume, return_lag1,
            return_daily, close_shift_3, target_3, target_direction_3,close_shift_5, target_5, target_direction_5, close_shift_10, target_10, close_shift_15,target_15,target_direction_15,close_shift_20,
            target_20, target_direction_20, macd,signal,macd..1:signal..1,hma_9:sma_50,DIp:ADX,
            dn,mavg,up,sar,max_high,min_low,close_month,open_month,month_yr,pp,s1,r1,s2,r2,s3,r3))
...
saveRDS(gb_mod_10,"SP500_gbml_trendpick_10_aapl.rds")

library(vip)
gb_mod_10 %>%
  vip(geom = "point",num_features = 50)

test_pred_10_rf <- predict(gb_mod_10, stock_test_10, type = "prob") %>%
...
saveRDS(gb_mod_15,"SP500_gbml_trendpick_15_aapl.rds")

library(vip)
gb_mod_15 %>%
  vip(geom = "point",num_features = 50)

test_pred_15_rf <-  predict(gb_mod_15, stock_test_15, type = "prob")%>%
...
saveRDS(gb_mod_20,"SP500_gbml_trendpick_20_aapl.rds")

library(vip)
gb_mod_20 %>%
  vip(geom = "point",num_features = 50)

test_pred_20_rf <- predict(gb_mod_20, stock_test_20, type = "prob") %>%
...
rf_mod_3 <- readRDS("SP500_gbml_trendpick_3_aapl.rds")
...
rf_mod_5 <- readRDS("SP500_gbml_trendpick_5_aapl.rds")
...
rf_mod_10 <- readRDS("SP500_gbml_trendpick_10_aapl.rds")
...
rf_mod_15 <- readRDS("SP500_gbml_trendpick_15_aapl.rds")
...
rf_mod_20 <- readRDS("SP500_gbml_trendpick_20_aapl.rds")
...
stock_pred_gb_symbol <- stock_pred_rf_symbol
