...
sp_500_1 <- sp_500_1 %>%
  mutate(target_direction_15 = case_when(
    target_15 > -5.5 & target_15 < 5.5 ~ "neutral",
    target_15 <= -5.5 ~ "sell",
    target_15 >=5.5 ~ "buy"
  ))
...
stock_ml_15$target_direction_15 <- factor(stock_ml_15$target_direction_15, levels=c("buy","neutral","sell"))


test_pred_15_rf <-  predict(rf_mod_15, stock_ml_15, type = "prob")%>%
  bind_cols(predict(rf_mod_15, stock_ml_15)) %>%
  bind_cols(select(stock_ml_15, target_direction_15)) %>%
  glimpse()
...
test_pred_15_rf <- bind_cols(test_pred_15_rf, select((sp_500_1 %>% na.omit()), date, symbol))

glimpse(sp_500_1)%>% na.omit()
